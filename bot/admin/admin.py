...
async def admin_process_add_product(call: CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.answer(
        text='Для начала укажите имя товара: ', reply_markup=cancel_kb_inline())
...
async def admin_process_name(message: Message, state: FSMContext):
    ...
    msg = await message.answer(
        text='Теперь дайте короткое описание товару: ',
...
async def admin_process_description(message: Message, state: FSMContext, session_without_commit: AsyncSession):
    ...
    msg = await message.answer(
        text='Теперь выберите категорию товара: ',
          reply_markup=catalog_admin_kb(catalog_data))
    await state.update_data(last_msg_id=msg.message_id)
...
async def admin_process_category(call: CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.edit_text(
        text='Введите цену товара: ', reply_markup=cancel_kb_inline())
    await state.update_data(last_msg_id=msg.message_id)
...
async def admin_process_confirm_add(call: CallbackQuery, state: FSMContext, session_with_commit: AsyncSession):
    ...
    await bot.delete_message(
        chat_id=call.from_user.id, message_id=product_data['last_msg_id'])
    del product_data['last_msg_id']
    await ProductDao.add(
        session=session_with_commit, values=ProductModel(**product_data))
    await call.message.answer(
        text='Товар успешно добавлен в базу данных!', reply_markup=admin_kb())
