...
async def cmd_start(message: Message, session_with_commit: AsyncSession):
    ...
    await message.answer(
        f'🎉 <b>Благодарим за регистрацию!</b>. Теперь выберите необходимое действие.',
            reply_markup=main_user_kb(user_id))
...
async def page_about(call: CallbackQuery):
    ...
    await call.message.answer(
        text=(
            ...
            '🛍️ Здесь вы можете работать с телеграм-магазином, '
            'ознакомиться с функциональностью и механизмами взаимодействия с пользователем.\n\n'
            '📚 Этот проект - это отличный способ для работы с телеграмм оплатами'
...
async def page_about(call: CallbackQuery, session_without_commit: AsyncSession):
    ...
    purchases = await UserDAO.get_purchase_statistics(
        session=session_without_commit, telegram_id=call.from_user.id)
